...
// Extracted from: public/landing-v2/CYB_Copy.js
// Centralized copy architecture. Pure data + accessor methods. No DOM, no state.
//
// Layers:
...
import type { BmiCategory } from "./cyb-calc";
...
export interface CompletQuestion {
  title: string;
  sub: string;
  note?: string;
}

export interface CompletTransition {
  title: string;
  body: string;
}
...
export const COPY = {
  ...
  // ── ROUTE (shared) ──────────────────────────────────────────────────
  route: {
    labels: ['Postpartum','Divorț/Separare','Schimbări hormonale','Burnout','Când pierzi pe cineva drag','General'] as string[],
    get(i: number): string { return this.labels[i] || this.fallbackLabel; },
  ...
  shared: {
    ...
    goalNames: ['slăbire','tonifiere','energie','sănătate'] as string[],
    gaugeLabels: ['Sub','Normal','Supra','Obez'] as string[],
  },
  ...
  } as Record<string, MetabolicProfile>,

  // ── FALLBACK ────────────────────────────────────────────────────────
  fallback: {
    metabolicProfile: { name:'Profil Echilibrat', color:'var(--teal-glow)', desc:'Ai o bază bună. Planul se concentrează pe optimizare: nutriție precisă, antrenament structurat, consistență.' } as MetabolicProfile,
  },
  ...
  mini: {
    ...
    emotional: {
      ...
      phase1: {
        name(d: { name?: string }): string { return 'Bun venit, ' + (d.name || '') + '! Hai să vedem ce poate face CYB pentru tine.'; },
        age(d: { age?: number | string }): string { return 'Perfect. La ' + (d.age || '') + ' de ani, corpul tău are nevoi specifice — și noi le înțelegem.'; },
        measures(d: unknown, bmi?: number): string { return 'Am calculat. IMC-ul tău este ' + (bmi ? bmi.toFixed(1) : '—') + ' — asta ne spune multe. Hai să continuăm.'; },
        activity(): string { return 'Înțeleg. Fiecare nivel de activitate are abordarea lui — programul tău va ține cont de asta.'; },
        goal(d: { goal?: number }): string {
          const goals = ['Slăbirea','Tonifierea','Energia','Sănătatea'];
          return (goals[d.goal ?? -1] || 'Obiectivul tău') + ' este un drum pe care l-am mai parcurs cu sute de femei. Nu ești singură.';
        },
        moment(d: { moment?: number }): string {
          const msgs = [
          ...
          const msg = msgs[d.moment ?? -1] || msgs[5];
          return msg + ' Am pregătit ceva special pentru tine.';
        },
      } as Record<string, (...args: unknown[]) => string>,
      get(key: string, d: unknown, extra?: unknown): string {
        const fn = this.phase1[key];
        if (typeof fn === 'function') { try { return fn(d, extra); } catch { return this.fallbackMessage; } }
    ...
    ui: {
      welcome: {
        ...
        freeItems: ['IMC-ul tău calculat cu vizualizare','Recomandări personalizate pe obiectiv','Program antrenament 1 săptămână','Acces grup WhatsApp CYB'] as string[],
        blurHeading: 'Ce vei vedea blurat (teaser):',
        blurItems: ['Metabolismul tău bazal (BMR)','Necesarul caloric zilnic (TDEE)','Proiecție progres 12 săptămâni','Macro-uri zilnice (proteine/carbs/grăsimi)','Greutate ideală estimată','Profil metabolic pe traseul tău'] as string[],
      },
      ...
      results: {
        headerTitle(name?: string): string { return 'Rezultatele tale, ' + (name || ''); },
        ...
        recoBody(bmi: string | number, goal: string, age: string | number): string { return 'Pe baza IMC-ului tău de ' + bmi + ' și a obiectivului de <strong style="color:white">' + goal + '</strong>, recomandăm o abordare graduală, bazată pe nutriție adaptată și mișcare progresivă. La ' + age + ' de ani, corpul tău răspunde cel mai bine la consistență, nu la intensitate.'; },
        freeItems: ['Program antrenament generic 1 săptămână (PDF)','Ghid Cumpărături Sănătoase (PDF)','Acces grup WhatsApp CYB'] as string[],
        ...
        macroLabels: ['Proteine','Carbohidrați','Grăsimi'] as string[],
        projHeading: 'Proiecție progres',
        projBody(deficit: string | number, weeks: string | number, target: string | number): string { return 'Dacă menții un deficit de 500 kcal/zi (' + deficit + ' kcal/zi), în <strong style="color:white">' + weeks + ' săptămâni</strong> poți ajunge la <strong style="color:var(--teal-glow)">' + target + ' kg</strong>.'; },
        idealHeading: 'Greutate ideală estimată',
        idealDesc(h: string | number): string { return 'interval sănătos pentru înălțimea ta de ' + h + ' cm'; },
      },
      upgrade: {
        ...
        footer(route: string): string { return 'Planurile de la 49€ includ chestionarul complet de 27 întrebări<br>pentru personalizare reală pe traseul tău <strong style="color:var(--gold)">' + route + '</strong>'; },
  ...
  complet: {
    ...
    } as Record<string, CompletQuestion>,
    ...
    } as Record<string, CompletTransition>,
    ...
    ui: {
      ...
      results: {
        ...
        headerTitle(name?: string): string { return (name||'') + ', am terminat analiza — iată ce am descoperit <em style="color:var(--teal-glow)">despre tine</em>'; },
        routePrefix: 'Traseu',
        ...
        timeOptions: ['<15 min','15-20 min','20-30 min','30-45 min','45+ min'] as string[],
        expOptions: ['Începător','Începător+','Intermediar','Avansat'] as string[],
        mealOptions: ['1–2','3','3+gustări','4–5 mici','Fără ritm'] as string[],
        budgetOptions: ['<150 RON','150-250','250-400','400+'] as string[],
        allergyOptions: ['Lactoză','Gluten','Nuci','Ouă','Pește','Soia','Fără'] as string[],
        cookTimeOptions: ['<15 min','15-30 min','30-45 min','45+ min','Fără gătit'] as string[],
        finalHeading: 'Planul tău e gata să fie construit',
        finalBody(profileName: string, profileColor: string): string { return 'Toate răspunsurile tale alimentează motorul nostru de personalizare care va genera:<br><strong style="color:white">raportul tău + plan alimentar + program antrenament</strong><br>— totul construit pe profilul tău <strong style="color:' + profileColor + '">' + profileName + '</strong>.'; },
        ...
        // N10: Custom duration option copy
        customOption: {
          heading: 'Sau alege durată personalizată',
          desc: 'Plan alimentar pe exact câte zile ai nevoie — cu aceleași rețete personalizate.',
          presetsLabel: 'Alege rapid:',
          presets: [14, 20, 30, 60] as number[],
          inputLabel: 'zile',
          pricePerDay: 7,
          priceLabel: 'Preț:',
          minDays: 14,
          maxDays: 90,
          buyLabel: 'Cumpără planul personalizat →',
          errorMin: 'Minim 14 zile',
          errorMax: 'Maxim 90 zile',
          errorInvalid: 'Introdu un număr valid',
        },
  ...
  // ── ACCESSORS (fallback) ────────────────────────────────────────────
  q(id: string): CompletQuestion { return this.complet.questions[id] || { title: id, sub: '', note: undefined }; },
  t(id: string): CompletTransition { return this.complet.transitions[id] || { title: '', body: '' }; },
};
