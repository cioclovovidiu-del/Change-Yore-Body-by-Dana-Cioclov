...
const FULFILLMENT_VERSION = "C6-1.0.0";

// ── C6: Server-side profile extraction from Stripe metadata ─────────
...
function extractProfile(
  ...
  // Validate essentials for calculation
...
// ── C6: Metabolic profile report HTML (Essential auto-deliverable) ───
function buildMetabolicReportHtml(
    ...
    <div style="background:#141e29;border-radius:16px;padding:28px 24px;border:1px solid rgba(201,168,76,0.15);">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:4px 0;">
              <span style="color:#999;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;">IMC (Body Mass Index)</span>
            </td>
          ...
          <tr>
            <td colspan="2" style="padding:6px 0 0;color:#666;font-size:12px;">
              Greutate ideală pentru tine: <strong style="color:#e0e0e0;">${ideal.low}–${ideal.high} kg</strong>
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="background:rgba(255,255,255,0.03);border-radius:10px;padding:16px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.06);">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:4px 0;">
              <span style="color:#999;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;">Metabolism bazal (BMR)</span>
            </td>
            <td style="padding:4px 0;text-align:right;">
          ...
          <tr>
            <td colspan="2" style="padding:6px 0 0;color:#666;font-size:12px;">
              Caloriile de care ai nevoie doar pentru funcții vitale (în repaus complet).
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="background:rgba(255,255,255,0.03);border-radius:10px;padding:16px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.06);">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:4px 0;">
              <span style="color:#999;font-size:12px;text-transform:uppercase;letter-spacing:0.06em;">Consum zilnic total (TDEE)</span>
            </td>
            <td style="padding:4px 0;text-align:right;">
          ...
          <tr>
            <td colspan="2" style="padding:6px 0 0;color:#666;font-size:12px;">
              Caloriile totale consumate zilnic, inclusiv activitatea fizică.
            </td>
          </tr>
        </table>
      </div>
      ...
      <div style="text-align:center;padding-top:12px;border-top:1px solid rgba(255,255,255,0.06);">
        ...
        <a href="https://wa.me/40721333040?text=Bun%C4%83%20Daniela,%20am%20primit%20raportul%20metabolic%20și%20am%20câteva%20întrebări." style="display:inline-block;padding:10px 24px;background:rgba(37,211,102,0.12);color:#25D366;border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;border:1px solid rgba(37,211,102,0.2);">Scrie pe WhatsApp →</a>
      </div>
    </div>
...
// ── C3/C5/C6: Fulfillment trigger (returns DeliveryResult) ──────────
async function triggerFulfillment(
  ...
  // 4. C6: Metabolic profile report (Essential auto-deliverable)
  if (
    resend &&
    ...
    try {
      const reportHtml = buildMetabolicReportHtml(payload, profile);
      const { error } = await resend.emails.send({
        from: "Change Your Body <onboarding@resend.dev>",
        to: payload.customerEmail,
