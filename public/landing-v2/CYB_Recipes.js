...
// Exports: RECIPES, buildDayPlan, filterRecipes, shoppingList,
//          formatDayPlanHtml, formatShoppingListHtml, MEAL_SLOTS
// =============================================================================
...
var PREP_TIME_MAP = {0:15, 1:30, 2:45, 3:999, 4:10};  // last: "nu gătesc" → only very quick/no-cook
...
function filterRecipes(mealType, profile, ans) {
  ...
  var allergyExclude = [];
  var allergies = ans.q19b || [];
  for (var ai = 0; ai < allergies.length; ai++) {
    var kw = ALLERGY_KEYWORDS[allergies[ai]];
    if (kw) { for (var ak = 0; ak < kw.length; ak++) allergyExclude.push(kw[ak]); }
  }

  // B3: Resolve max prep time from q19c
  var maxPrepMin = ans.q19c !== undefined ? (PREP_TIME_MAP[ans.q19c] || 999) : 999;

  // B4: Parse refused foods from q19d (free-text textarea)
  var refusedWords = [];
  var refusedRaw = (ans.q19d || '').toLowerCase().trim();
  if (refusedRaw) {
    // Split by comma, semicolon, newline, "și", or "si"
    var parts = refusedRaw.split(/[,;\n]+|\s+(?:și|si)\s+/);
    for (var ri = 0; ri < parts.length; ri++) {
      var word = parts[ri].trim();
  ...
  for (var j = 0; j < RECIPES.length; j++) {
    ...
    if (allergyExclude.length > 0) {
      ...
      for (var ig = 0; ig < ingList.length && !hasAllergen; ig++) {
        var ingLow = ingList[ig].toLowerCase();
        for (var ae = 0; ae < allergyExclude.length && !hasAllergen; ae++) {
    ...
    if (refusedWords.length > 0) {
      ...
      var rIngList = r.ingredients || [];
      for (var rw = 0; rw < refusedWords.length && !isRefused; rw++) {
        if (titleLow.indexOf(refusedWords[rw]) !== -1) { isRefused = true; break; }
        for (var rg = 0; rg < rIngList.length && !isRefused; rg++) {
...
function formatDayPlanHtml(plan) {
  ...
  for (var i = 0; i < order.length; i++) {
    ...
    if (slot.recipe) {
      ...
      if (slot.recipe.ingredients) html += '<p style="color:var(--muted);font-size:11px;margin:4px 0 0;">Ingrediente: ' + slot.recipe.ingredients.join(', ') + '</p>';
      if (slot.recipe.instructions) html += '<p style="color:var(--text);font-size:11px;margin:2px 0 0;font-style:italic;opacity:0.8;">' + slot.recipe.instructions + '</p>';
      html += '</div>';
...
function shoppingList(plan) {
  ...
  var map = {}; // ingredient name (lowercase) → {name (original case), count, recipes[]}
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var ingList = recipe.ingredients || [];
...
// ── FORMAT SHOPPING LIST (HTML helper) ───────────────────────────────
// Returns HTML string for displaying a shopping list.
function formatShoppingListHtml(list) {
  if (!list || list.length === 0) return '<p style="color:var(--muted)">Lista de cumpărături nu este disponibilă.</p>';

  var html = '<div class="shopping-list" style="text-align:left;">';
  html += '<p style="color:var(--accent);font-weight:600;margin-bottom:10px;">🛒 Lista de cumpărături — Ziua model</p>';
  html += '<div style="columns:2;column-gap:16px;font-size:13px;">';

  for (var i = 0; i < list.length; i++) {
    var item = list[i];
    html += '<div style="break-inside:avoid;margin-bottom:4px;color:var(--text);">';
    html += '• ' + item.name;
    if (item.count > 1) html += ' <span style="color:var(--muted);font-size:11px;">(×' + item.count + ')</span>';
    html += '</div>';
  }

  html += '</div>';
  html += '<p style="color:var(--muted);font-size:11px;margin-top:8px;">' + list.length + ' ingrediente unice</p>';
  html += '</div>';
  return html;
}
